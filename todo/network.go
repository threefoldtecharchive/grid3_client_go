...
func NewNetworkDeployer(manager deployer.DeploymentManager, target TargetNetwork) (NetworkDeployer, error) {
	...
	for _, nodeID := range k.Nodes {
		...
		for _, wl := range dl.Workloads {
			if wl.Name.String() == target.Name && wl.Result.State == gridtypes.StateOk {
	...
	for nodeID, contractID := range oldDeployments {
		...
		wlID, err := dl.Get(gridtypes.Name(target.Name))
...
func (network *TargetNetwork) Stage(
	...
	k, err := NewNetworkDeployer(apiClient.Manager, *network)
	if err != nil {

...
func constructTestDeployer(ctrl *gomock.Controller) DeploymentDeployer {
	pool := mocks.NewMockNodeClientGetter(ctrl)
...
func TestDeploymentSyncDeletedContract(t *testing.T) {
	...
	d.ID = id
	assert.NoError(t, d.Sync(context.Background(), sub, d.TFPluginClient))
	assert.Empty(t, d.ID)
...
func TestDeploymentGenerateDeployment(t *testing.T) {
	...
	wls = append(wls, wl)
	wls = append(wls, d.Disks[0].GenerateWorkload())
	wls = append(wls, d.Disks[1].GenerateWorkload())
	wls = append(wls, d.ZDBs[0].GenerateWorkload())
	wls = append(wls, d.ZDBs[1].GenerateWorkload())
...
func TestDeploymentSync(t *testing.T) {
	...
	*wl.Workload = wl.WithResults(gridtypes.Result{
	...
	d.deployer.(*mocks.MockDeployer).EXPECT().
	...
	manager.EXPECT().DeleteDeploymentNetworkHostIDs(d.NetworkName, d.Node, d.ID).AnyTimes()
